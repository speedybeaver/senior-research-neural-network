...
def name(number):
    i = CLASSIFICATIONS.index(number)
    return NAMES[i]
...
for trial in range(NUM_TRIALS):
    for classification in CLASSIFICATIONS:
        ...
        with open('donegan_data.csv', 'a', newline='') as csvfile:
        ...
        if (CLASSIFICATION < 7):
            input(f"Press enter to continue to {name(CLASSIFICATION+1)}")
        else:
            print("Thank you for the trial.")
    input(f"Press enter to continue to loop #{trial+2}")
ser.close()